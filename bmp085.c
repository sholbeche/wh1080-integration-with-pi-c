...
int read_bmp085(float* alt, float* temp, float* pressureSea, float* pressureAlt) {
...
//		printf("%d %d %d %d %d\n", b1, b2, mb, mc, md);
	...
	for(idx=0; idx < 2; idx++) {
		...
		p0 += (float)p / powf(1.0f - (*alt / 44330), 5.255f);
	...
	printf("Pressure pR (reading): %0.1f hPa\n", pA/100);
	*pressureSea = pressure_hpa;
	*pressureAlt = pA /100;
